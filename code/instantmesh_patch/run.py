...
# Ensure env-local executables (e.g., ninja) are discoverable when launched via
# absolute python path without conda activation.
if os.environ.get("CUDA_HOME", "").startswith(":/") or not os.environ.get("CUDA_HOME"):
    os.environ["CUDA_HOME"] = "/usr/local/cuda"
os.environ["PATH"] = (
    f"{os.path.dirname(sys.executable)}:/usr/local/cuda/bin:"
...
local_pipeline_dir = os.path.join(os.path.dirname(__file__), "zero123plus")
custom_pipeline = (
    local_pipeline_dir
    if os.path.exists(os.path.join(local_pipeline_dir, "pipeline.py"))
    else "zero123plus"
)
